 ...
 */
export const CHAT_MARKDOWN_BLOCK_RHYTHM = chatClassNames(
  "chat-markdown",
  ...
  "[&_pre]:my-2 [&_pre]:overflow-x-auto [&_pre]:rounded-lg [&_pre]:border [&_pre]:border-border/60 [&_pre]:bg-bg-surface/60 [&_pre]:px-3 [&_pre]:py-2 [&_pre]:font-mono [&_pre]:text-[13px] [&_pre]:leading-6",
...
export const CHAT_MARKDOWN_PROSE_CLASS =
  "font-chat text-[14px] leading-7 text-fg";

export const CHAT_ASSISTANT_MARKDOWN_CLASS = chatClassNames(
  CHAT_MARKDOWN_PROSE_CLASS,
  CHAT_MARKDOWN_BLOCK_RHYTHM,
);

export const CHAT_THOUGHT_MARKDOWN_CLASS = chatClassNames(
  "chat-markdown--thought font-chat text-[13px] leading-6 text-fg-muted",
  CHAT_MARKDOWN_BLOCK_RHYTHM,
);
...
const baseComponents = {
  ...
  pre: ({
    ...
    <pre
      {...rest}
      className="my-2 overflow-x-auto rounded-lg border border-border/60 bg-bg-surface/60 px-3 py-2 font-mono text-[12px] leading-5"
    >
      {children}
    </pre>
  ),
  code: ({ className, children, ...rest }: HTMLAttributes<HTMLElement>) =>
    className?.startsWith("language-") ? (
      ...
      <code
        {...rest}
        className="rounded bg-bg-surface/70 px-[0.35em] py-[0.1em] font-mono text-[0.9em] text-fg"
      >
        {children}
      </code>
    ),
