...
function settleThoughtTiming(turn: AgentUITurnState | undefined, event: OpenMAEvent): void {
  if (!turn) return;
  const thought = [...turn.items].reverse().find(
    (item): item is AgentUIMessageItem => item.kind === "thinking" && !item.endedAt,
  );
  if (!thought?.startedAt) return;
  const data = eventData(event);
...
export function reduceAgentUIEvent(
  ...
  settleThoughtTiming(
    turn ?? (event.type.startsWith("session.") && next.activeTurnId ? next.turns[next.activeTurnId] : undefined),
    event,
  );
