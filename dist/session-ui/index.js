 ...
 */
export function SessionTurnFrame({ turnId, sessionId, promptText, promptAnnotation, status, errorMessage, errorNotice, labels, children, className = "", }) {
    const isStreaming = status === "running";
    const statusNode = (status === "error" || status === "errored") && errorNotice !== undefined
        ? errorNotice
        : renderStatus(status, errorMessage, labels);
    return (_jsxs("article", { className: `group/turn reveal-in mb-6 space-y-2 ${className}`.trim(), "data-turn-id": turnId, "data-session-turn-status": status, children: [promptText || promptAnnotation ? (_jsxs("div", { className: "group is-user ml-auto flex w-full max-w-[95%] flex-col items-end gap-2", "data-session-turn-prompt": "true", children: [promptText ? (_jsx("div", { className: "is-user:dark ml-auto flex w-fit min-w-0 max-w-full flex-col gap-2 overflow-hidden rounded-lg bg-secondary px-4 py-3 text-sm text-foreground", children: _jsx("p", { className: "whitespace-pre-wrap", children: promptText }) })) : null, promptAnnotation ? (_jsx("div", { className: "flex min-w-0 max-w-full items-center gap-1.5 text-xs text-fg-muted", "data-session-turn-prompt-annotation": "true", children: promptAnnotation })) : null] })) : null, _jsx("div", { className: "min-w-0", "data-annotatable-response": true, "data-annotation-ready": !isStreaming, "data-source-session-id": sessionId, "data-source-turn-id": turnId, children: _jsxs("div", { className: "min-w-0 space-y-2", "data-session-turn-response": "true", children: [children, statusNode] }) })] }));
}
