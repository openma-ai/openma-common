...
export function decodeChildActivities(value, tool) {
    ...
    const ids = Array.isArray(targets) ? targets.filter((id) => typeof id === "string") : [];
    ...
    if (operation && ids.length > 0) {
        ...
        return ids.flatMap(id => {
            ...
            if (operation === "closeAgent")
                return tool.status === "completed" ? [{ id, status: "closed" }] : [];
            ...
            if (operation !== "wait")
                return [];
            if (status === "completed" || state.completed !== undefined)
                return [{ id, status: "completed" }];
            if (status === "errored" || status === "failed" || state.failed !== undefined || state.error !== undefined)
                return [{ id, status: "failed", error: string(state.message ?? state.error ?? state.failed) }];
            if (status === "interrupted" || status === "cancelled" || state.cancelled === true)
...
//# sourceMappingURL=acp-subagents.js.map