import { reduceWorkItemEvent, } from "../session-events/openma.js";
export const AGENT_UI_STATE_VERSION = "oma.agent-ui.v1";
export function createAgentUIState(sessionId) {
    return {
...
function cloneState(state) {
    return {
        ...
        turns: Object.fromEntries(Object.entries(state.turns).map(([id, turn]) => [
            id,
            {
                ...turn,
        ...
        workItems: Object.fromEntries(Object.entries(state.workItems).map(([id, item]) => [
            id,
            { ...item, output: [...item.output] },
        ])),
        planOrder: [...state.planOrder],
        plans: Object.fromEntries(Object.entries(state.plans).map(([id, plan]) => [
            id,
            { ...plan, entries: plan.entries.map((entry) => ({ ...entry })) },
        ])),
        callbackOrder: [...state.callbackOrder],
        callbacks: Object.fromEntries(Object.entries(state.callbacks).map(([id, callback]) => [
            id,
            { ...callback },
        ])),
        ...(state.usage ? { usage: { ...state.usage } } : {}),
        outcomeOrder: [...state.outcomeOrder],
        outcomes: Object.fromEntries(Object.entries(state.outcomes).map(([id, outcome]) => [id, { ...outcome }])),
        seenEventIds: { ...state.seenEventIds },
    };
}
function toWorkItemSnapshot(item) {
    return {
        id: item.id,
        kind: item.kind,
        status: item.status,
...
function toAgentUIWorkItem(item) {
    return {
        id: item.id,
        kind: item.kind,
        status: item.status,
...
function applyWorkItemProjection(state, previous, event) {
    const registry = reduceWorkItemEvent({
        items: new Map(Object.entries(previous.workItems).map(([id, item]) => [
            id,
    ...
    for (const id of ids) {
        if (!state.workItemOrder.includes(id))
            state.workItemOrder.push(id);
    }
    state.workItems = Object.fromEntries([...registry.items].map(([id, item]) => [id, toAgentUIWorkItem(item)]));
}
function planId(event) {
    if (!event.data || typeof event.data !== "object")
        return undefined;
    const value = event.data.plan_id;
    return typeof value === "string" && value.length > 0
        ? value
        : event.turn_id;
}
function mergePlanEntries(previous, incoming) {
...
function applyPlanProjection(state, event) {
    ...
    if (event.type === "plan.updated") {
        ...
        if (!state.planOrder.includes(id))
            state.planOrder.push(id);
    }
    else if (event.type === "plan.completed") {
        const previous = state.plans[id];
        if (previous)
            state.plans[id] = { ...previous, status: "completed" };
    }
...
function applyCallbackProjection(state, event) {
    ...
    if (!event.data || typeof event.data !== "object")
        return;
    const data = event.data;
    const callbackId = (typeof data.callback_id === "string" && data.callback_id.length > 0) || typeof data.callback_id === "number"
    ...
    if (!state.callbackOrder.includes(id))
        state.callbackOrder.push(id);
}
function applyOutcomeProjection(state, event) {
    ...
    if (!state.outcomeOrder.includes(id))
        state.outcomeOrder.push(id);
}
function requireTurn(state, event) {
...
function upsertMessage(turn, event, input) {
...
function uniqueMessageItemId(turn, sourceId) {
    if (!turn.items.some((item) => item.id === sourceId))
        return sourceId;
    let segment = 2;
...
function upsertTool(turn, event, status) {
    if (!event.data || typeof event.data !== "object")
        return;
    const data = event.data;
    ...
    let item = turn.items.find((candidate) => candidate.kind === "tool" && candidate.id === data.tool_call_id);
    if (!item) {
        item = {
    ...
    if (data.output !== undefined) {
        if (data.output.append)
            item.outputs.push(data.output);
        else
            item.outputs = [data.output];
    }
}
...
function stopReasonType(event) {
    if (!event.data || typeof event.data !== "object")
        return undefined;
    const stopReason = event.data.stop_reason;
...
export function reduceAgentUIEvent(state, event) {
    ...
    switch (event.type) {
        ...
        case "turn.completed":
            ...
            if (next.activeTurnId === event.turn_id)
                next.activeTurnId = undefined;
            break;
        case "turn.failed":
            if (turn) {
        ...
        case "tool.started":
            if (turn)
                upsertTool(turn, event, "in_progress");
            break;
        case "tool.progress":
            if (turn)
                upsertTool(turn, event, "in_progress");
        ...
        case "tool.completed":
            if (turn)
                upsertTool(turn, event, "completed");
            break;
        case "tool.failed":
            if (turn)
                upsertTool(turn, event, "failed");
            break;
        case "tool.cancelled":
            if (turn)
...
export function replayAgentUIEvents(sessionId, events) {
    const ordered = events
        ...
        if (left.event.seq === undefined)
            return 1;
        if (right.event.seq === undefined)
            return -1;
...
export function createAgentUIStore(sessionId) {
    ...
    return {
        ...
        subscribeTurnStream: (turnId, listener) => {
            ...
            if (thought)
                listener({ kind: "thought", text: thought });
            const assistant = accumulatedTurnStream(state, turnId, "assistant");
            if (assistant)
                listener({ kind: "assistant", text: assistant });
            return () => {
                ...
                if (current.size === 0)
                    streamSubscribers.delete(turnId);
            };
        },
    };
}
function streamProjection(event) {
...
function accumulatedTurnStream(state, turnId, kind) {
    if (!turnId)
        return "";
    ...
    return turn.items.flatMap((item) => {
        if (item.kind === "tool")
            return [];
        if (kind === "thought")
            return item.kind === "thinking" ? [item.text] : [];
        return item.kind === "message" && item.role === "assistant" ? [item.text] : [];
    }).join("");
}
function opensStreamSegment(state, event, kind) {
    if (!event.turn_id)
        return false;
    const tail = state.turns[event.turn_id]?.items.at(-1);
    if (!tail || tail.kind === "tool")
        return true;
    if (kind === "thought") {
