...
function validateSelection(value) {
    if (!safeName.test(value.id) || !packageName.test(value.package))
        throw new Error("Invalid ACP release identity");
    if (!exactVersion.test(value.version))
        throw new Error("ACP release requires an exact version (no tags or ranges)");
}
...
export function validateNpmAcpRelease(value) {
    if (!value || typeof value !== "object")
        throw new Error("Invalid ACP release record");
    const r = value;
    for (const key of ["id", "version", "package", "tarball", "integrity", "bin", "digest"]) {
    ...
    if (r.schema !== "openma.acp.npm.v1" || !httpsUrl(r.tarball)
        ...
        || r.digest !== fingerprint(r))
        throw new Error("Invalid ACP release digest or metadata");
...
export async function resolveNpmAcpRelease(selection, options = {}) {
    validateSelection(selection);
    const response = await (options.fetch ?? fetch)(`https://registry.npmjs.org/${encodeURIComponent(selection.package)}/${encodeURIComponent(selection.version)}`, { signal: signalWithTimeout(options.signal, 30_000) });
    if (!response.ok)
        throw new Error(`ACP release ${selection.id}@${selection.version}: registry HTTP ${response.status}`);
    const data = await response.json();
    if (data.name !== selection.package || data.version !== selection.version)
        throw new Error("ACP registry release identity mismatch");
    ...
    const record = { schema: "openma.acp.npm.v1", ...selection,
...
export async function prepareNpmAcpRelease(input, options) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`ACP artifact download HTTP ${response.status}`);
        const bytes = Buffer.from(await response.arrayBuffer());
        const integrity = `sha512-${createHash("sha512").update(bytes).digest("base64")}`;
        if (integrity !== release.integrity)
            throw new Error("ACP artifact integrity mismatch");
...
async function verifyInstalled(directory, release) {
    ...
    if (metadata.name !== release.package || metadata.version !== release.version)
        throw new Error("ACP installed package identity mismatch");
    const binPath = typeof metadata.bin === "string" ? metadata.bin : metadata.bin?.[release.bin];
    if (typeof binPath !== "string" || isAbsolute(binPath))
        throw new Error("ACP release executable is invalid");
...
async function readPrepared(directory, release) {
    let saved;
    try {
        saved = await readFile(join(directory, "release.json"), "utf8");
    }
    catch (error) {
    ...
    if (validateNpmAcpRelease(JSON.parse(saved)).digest !== release.digest)
        throw new Error("ACP cached release identity mismatch");
