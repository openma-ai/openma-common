...
import { digest, identity, safeRelative, url, launchOptions, platformKey, download, installAtomically, childEnvironment } from "./shared.js";
export function binaryFormat(address) {
    const path = new URL(address).pathname.toLowerCase();
    if (/\.(dmg|pkg|deb|rpm|msi|appimage)$/.test(path))
        throw new Error("Installer packages are not ACP binary distributions");
    if (/\.(tar\.gz|tgz)$/.test(path))
        return "tar.gz";
    if (/\.(tar\.bz2|tbz2)$/.test(path))
        return "tar.bz2";
    if (/\.(tar\.xz|txz)$/.test(path))
        return "tar.xz";
...
export function resolveBinaryAcpRelease(input) {
    identity(input.id, input.version);
    if (typeof input.platform !== "string" || !/^[a-z0-9_-]+$/.test(input.platform))
        throw new Error("Invalid binary platform");
    url(input.archive);
    if (typeof input.sha256 !== "string" || !/^[a-fA-F0-9]{64}$/.test(input.sha256))
        throw new Error("Binary releases require a SHA-256 checksum");
    const format = input.format ?? binaryFormat(input.archive);
    if (!["raw", "zip", "tar", "tar.gz", "tar.bz2", "tar.xz"].includes(format))
        throw new Error("Unsupported binary archive format");
    const record = { schema: "openma.acp.binary.v1", id: input.id, version: input.version,
        platform: input.platform, archive: input.archive, sha256: input.sha256.toLowerCase(), format: format,
        command: safeRelative(input.command), ...launchOptions(input) };
    return { ...record, digest: digest(record) };
}
export function validateBinaryAcpRelease(value) {
    if (!value || typeof value !== "object")
        throw new Error("Invalid binary release");
    const input = value;
    const result = resolveBinaryAcpRelease(input);
    if (input.schema !== result.schema || input.digest !== result.digest)
        throw new Error("Invalid binary release digest");
    return result;
}
export async function prepareBinaryAcpRelease(input, options) {
    const release = validateBinaryAcpRelease(input);
    if (release.platform !== platformKey())
        throw new Error(`Binary platform ${release.platform} does not match ${platformKey()}`);
    return installAtomically(join(options.root, platformKey()), release, async (directory) => {
        ...
        if (createHash("sha256").update(bytes).digest("hex") !== release.sha256)
            throw new Error("Binary artifact integrity mismatch");
        ...
        if (release.format === "raw") {
            ...
            if (release.format === "tar.bz2" || release.format === "tar.xz") {
                const compressed = join(directory, "compressed");
            ...
            await tar.t({ file: archive, strict: true, onReadEntry(entry) {
                    try {
                        ...
                        if (!["File", "OldFile", "Directory"].includes(entry.type))
                            throw new Error("Binary archives cannot contain links or special files");
                    }
                    catch (error) {
                        invalid = error;
                    }
        ...
        if (rel.startsWith("..") || isAbsolute(rel) || !(await stat(command)).isFile())
            throw new Error("Binary executable path escapes the artifact");
